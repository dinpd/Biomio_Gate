from biomio.protocol.probes.plugins.openface_verify_plugin.openface_algorithm_interface \
    import OpenFaceAlgorithmInterface, training_job, verification_job
...
class OpenFaceVerifyPlugin(base_probe_plugin.BaseProbePlugin):
    _settings = dict(userID="0000000000000")
    ...
    def run_verification(self, data, callback=None):
        self._callback = callback
        self._data_validator(data)
        data.update({'settings': self._settings.copy()})
